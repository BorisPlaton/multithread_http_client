...
class TestWorkersManager:
    ...
    def workers_manager(self) -> WorkersManager:
        ...
        yield manager
        manager.url_statuses.restore()

    @pytest.fixture
    def storage(self, workers_manager):
        return workers_manager.url_statuses

    def test_if_url_is_discarded_or_downloaded_exception_raised(self, workers_manager, storage):
        downloaded_url = '/'
        storage.add_url_to_downloaded(downloaded_url, 'path')
        with pytest.raises(URLDataException):
            workers_manager.check_task_url(downloaded_url)
        discarded_url = '/new/one'
        storage.add_url_to_discarded(discarded_url, 'reason')
        with pytest.raises(URLDataException):
            workers_manager.check_task_url(discarded_url)

    def test_if_url_is_in_process_exception_is_not_risen(self, workers_manager, storage):
        in_process_url = '/'
        storage.add_url_to_in_process(in_process_url, 500)
        assert not workers_manager.check_task_url(in_process_url)

    def test_url_is_added_to_in_process_if_new_tasks_for_workers_are_pushed(self, workers_manager, storage):
        url_content_data = URLResourceData('/', 500)
        workers_manager.create_new_tasks_for_workers(url_content_data)
        url_data_in_process = storage.get_url(url_content_data.url)
        assert storage.is_in_process(url_data_in_process)
        assert url_data_in_process.summary_size == url_content_data.summary_size

    def test_task_queue_contains_new_tasks_when_tasks_are_created(self, workers_manager, storage):
        url_content_data = URLResourceData('/', 500)
    ...
    async def test_if_url_data_validation_exception_was_risen_url_added_to_discarded(
            self, get_url_mock: MagicMock, workers_manager, storage
    ):
        reason = 'some reason'
        ...
        url_data = storage.get_url(dispatched_url)
        assert storage.is_discarded(url_data)
    ...
    async def test_if_url_data_exception_was_risen_nothing_happen(
            self, check_url_mock: MagicMock, workers_manager, storage
    ):
